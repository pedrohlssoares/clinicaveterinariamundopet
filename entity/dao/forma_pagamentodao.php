...
class forma_pagamentoDao {
    ...
    public function read() {
        try {
            $pdo = conexao::conectar();
            $sql = "SELECT idforma_pagamento, tipo, descricao FROM forma_pagamento";
            $result = $pdo->query($sql);
            $lista = [];
            foreach($result as $linha) {
                $lista[] = new forma_pagamento(
                    $linha["idforma_pagamento"], $linha["tipo"], $linha["descricao"]
                );
            }
            conexao::desconectar();
            return $lista;
    ...
    public function readID($idforma_pagamento) {
        try {
            $pdo = conexao::conectar();
            $sql = "SELECT idforma_pagamento, tipo, descricao FROM forma_pagamento WHERE idforma_pagamento = ?";
            $query = $pdo->prepare($sql);
    ...
    public function update(forma_pagamento $forma_pagamento) {
        try {
            $pdo = conexao::conectar();
            $sql = "UPDATE forma_pagamento SET tipo = ?, descricao = ? WHERE idforma_pagamento = ?";
            $query = $pdo->prepare($sql);
            $query->execute([
                $forma_pagamento->tipo, $forma_pagamento->descricao, $forma_pagamento->idforma_pagamento
            ]);
            conexao::desconectar();
            return true;
        } catch (PDOException $exception) {
